...
#include <float.h>
#include <math.h>
// PLATFORM-INDEPENDENT PORT: use the portable Win32 shim instead of <windows.h>/
// <commctrl.h> so this machine's DSP compiles + runs on any platform (the Win32
// config dialog + "dock" become inert stubs; the app supplies its own editor).
#define STATIC_BUILD               // compile the machine IN and self-register
#include "buzz_platform.h"
#include "resource.h"
inline void ZeroMemory(void* ptr, int length) { memset(ptr, 0, (size_t)length); }
#include "mdk.h"
...
// STATIC_BUILD: expands to GetInfo()/CreateMachine()/unwieldy_register(), the last
// calling the host's RegisterMachine().  A file-scope object calls it at startup so
// the machine self-registers into the PatchKnob buzz host (no DLL loading).
DLL_EXPORTS(unwieldy_register)
// (the host calls unwieldy_register() explicitly via register_builtin_machines()
//  -- no self-init object, which would double-register once the TU is linked in.)
			...
			if(poolmi&&(HIWORD(wParam)==BN_CLICKED))
			{
				...
				if(hPool)
				{
					...
					for(int b=0;b<seln;b++)
					{
						...
						int c;
						for(c=b+1;c<seln;c++)if(seld[c]>seld[b])seld[c]--;
...
mi::mi()
{
	...
	for(int oi=0;oi<32;oi++)outbuf[oi]=NULL;   // CreateOutBufs() (in Init) fills these;
	                                           // NULL first so ~mi/DeleteOutBufs is safe
	                                           // even if the machine is destroyed pre-Init.
	...
	delete[] tehmilist;   // allocated with new milist[] -> must be delete[], not free()
	tehmilist=newml;
	micount++;
	/*for(i=0;i<micount;i++)
	{
...
{
	for(int i=0;i<32;i++)if(outbuf[i]){delete[] outbuf[i];outbuf[i]=NULL;}   // new float[] -> delete[]
}
		...
		if(pwave)
		{
			...
			if(vesize)
			{
				...
				else
				{
					...
					vecn+=(int)((float)fnstep/nsamp*65536)/*was *128: made env 128x too fast -> instant attack/release clicks; x now spans one playthrough*/;
				}
			}
			...
			if(pesize)
			{
				...
				else
				{
					...
					pecn+=(int)((float)fnstep/nsamp*65536)/*was *128: made env 128x too fast -> instant attack/release clicks; x now spans one playthrough*/;
				}
			...
			if(nesize)
			{
				...
				else
				{
					...
					necn+=(int)((float)fnstep/nsamp*65536)/*was *128: made env 128x too fast -> instant attack/release clicks; x now spans one playthrough*/;
				}
			...
			if(filt)
			{
				if(cesize)
				{
					...
					else
					{
						...
						cecn+=(int)((float)fnstep/nsamp*65536)/*was *128: made env 128x too fast -> instant attack/release clicks; x now spans one playthrough*/;
					}
					if(cc)cut*=cm;
					else cut=pcut*cm;
					cc=true;
				...
				if(resize)
				{
					...
					else
					{
						...
						recn+=(int)((float)fnstep/nsamp*65536)/*was *128: made env 128x too fast -> instant attack/release clicks; x now spans one playthrough*/;
					}
					res=pres*rm;
					cc=true;
				}
...
void mi::Init(CMachineDataInput * const pi)
{
	...
	{
		int i;
		for(i=0;i<32768;i++)sint[i]=(float)sin(EDU*2*PI*i);
...
bool mi::WorkMonoToStereo(float *pin, float *pout, int numsamples, int const mode)
{
	...
	int i;
	for(i=0;i<32;i++)ZeroMemory(outbuf[i],MAX_BUFFER_LENGTH*sizeof(float));
