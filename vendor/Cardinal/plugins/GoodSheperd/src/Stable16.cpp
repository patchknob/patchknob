...
struct Stable16 : Module
{
	...
	dsp::SequencerReset sequencerReset;
	...
	void process(const ProcessArgs &args) override
	{
		...
		if (running)
		{
			...
			{
				// External clock
				if (sequencerReset.processClock(clockTrigger.process(rescale(inputs[EXT_CLOCK_INPUT].getVoltage(), 0.1f, 1.f, 0.f, 1.f))))
					calculateNextIndex();
